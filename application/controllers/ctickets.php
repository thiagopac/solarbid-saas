...
class cTickets extends MY_Controller {
	...
	function index()
	{
		...
		$this->content_view = 'tickets/client/all';
	}
	...
	function create()
	{
		...
		}else
		{
			...
			$this->content_view = 'tickets/client/_ticket';
		}
	}
	function view($id = FALSE)
	{
		...
		$this->view_data['submenu'] = array();
		$this->content_view = 'tickets/client/view';
	...
	function article($id = FALSE, $condition = FALSE, $article_id = FALSE)
	{
		...
		switch ($condition) {
			case 'add':
				$this->content_view = 'tickets/client/_note';
				if($_POST){
					$config['upload_path'] = './files/media/';
					$config['encrypt_name'] = TRUE;
					$config['allowed_types'] = '*';
