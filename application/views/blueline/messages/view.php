<!-- VIEW INDIVIDUAL DE MENSAGEM - ADMIN -->
<?php if ($conversation) {
    $i = 0;
    foreach ($conversation as $value):
          $own = false;
    $unix = human_to_unix($value->time);
    if ('u' . $this->user->id == $value->sender) {
        $own = ' own';
    } else {
        $own = '-previous';
    }
...
<?php if ($i == '1' && $own != 'own') {
 ...
 <div class="message-content-menu">
              ...
              <!-- ESCONDER BOTÃO DE FAVORITAR MENSAGEM -->
              <!-- <a class="btn btn-primary ajax-silent" href="<?=base_url()?>messages/mark/<?=end($conversation)->id?>" role="button">
              <?php if (end($conversation)->status == 'Marked') {
            ?>
            ...
            ?>
              ...
              <?php
        if (end($conversation)->deleted == 0) {
            ?>
              <a class="btn btn-danger" href="<?=base_url()?>messages/delete/<?=end($conversation)->id?>" role="button"><i class="icon dripicons-trash"></i> <?=$this->lang->line('application_delete'); ?></a>
              <?php
        } ?>
    ...
    <p class="subtitle"><?=$this->lang->line('application_from'); ?> <a href="#">
    <?php if (($value->sender_u != null) && $filter = 'Deleted') {
            echo $value->sender_u;
        } else {
            echo $value->sender_c;
        } ?></a> <?=$this->lang->line('application_to'); ?> <a href="#"><?php if ($recipient != null) {
            echo $recipient;
        } else {
      ...
      "/>
            <?php
                ...
                if ($value->sender_u != null && $filter != 'Deleted') {
//                    echo $value->sender_u;
                    echo $value->sender_c;
                } else if ($value->sender_c != null && $filter != 'Deleted') {
//                    echo $value->sender_u;
                    echo $value->sender_c;
                }
