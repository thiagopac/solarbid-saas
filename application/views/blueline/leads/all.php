        ...
        <div id="kanban-page">
            ...
            <kanban-board :stages="stages" :blocks="getLeads" @update-block="updateBlock" @delete-block="deleteBlock">
                ...
                <div v-for="(block, index) in getLeads" :slot="block.id" v-cloak>
                    ...
                    <div>
                        <!--						<i class="icon dripicons-lock" style="font-size: 20px; color: orangered;" title="-->
                        <?//=htmlspecialchars($this->lang->line('application_private_lead'));?>
                        <!--"-->
                            ...
                            <div class="tab-pane tab1" v-if="openDetails == block.id" :class="(openDetails == block.id) ? 'in active animated slideInLeft' : '' ">
                                <ul class="details">
                                    ...
                                    <li v-if="block.address != '' || block.city != '' || block.zipcode != '' || block.country != ''">
										...
										</span> {{ block.address}} {{ block.city }} {{ block.zipcode}} {{ block.country }}
                                    </li>
                                    <li v-if="block.email != ''">
                                                    <span>
                                    ...
                                    <transition-group name="list" tag="ul" class="lead-comments" appear>
                                        <li v-for="comment in comments" v-bind:key="comment.id">
                                            ...
                                            <div class="lead-attachment" v-if="comment.attachment">
                                                <a v-if="isImage(comment.attachment_link)" :href="'<?=base_url()?>files/media/'+comment.attachment_link" :data-lightbox="'lead'+comment.lead_id" :data-title="comment.attachment">
                                ...
                                <transition-group name="list" tag="ul" class="lead-reminders" appear>
                                    <li v-for="(reminder, index) in reminders" v-bind:key="reminder.id">
                                        <div class="reminder-wrapper has-hover-options" :class="[(isDue(reminder.datetime) && reminder.done == 0) ? 'red' : 'green', (reminder.done == 1) ? 'gray' : '']">
                                            ...
                                            <p>
                                                <i @click="toggleReminder(reminder.id)" class="ionicons reminder-check" :class="(reminder.done == 1) ? 'ion-android-checkbox-outline' : 'ion-android-checkbox-outline-blank'" title="<?=$this->lang->line('application_done');?>"></i> {{ reminder.title }}</p>
                                        </div>
                                    </li>
                                </transition-group>
                            </div>
                    ...
                    <div class="row block-actions" :class="block.completed">
                        ...
                        <a class="col-xs-2 center" :class="(block.address != '' || block.city != '' || block.zipcode != '' || block.country != '') ? 'white' : 'transparent'" :href="(block.address != '' || block.city != '' || block.zipcode != '' || block.country != '') ? 'https://maps.google.com?q='+block.address+'+'+block.city+'+'+block.zipcode+'+'+block.country : ''" target="_blank" :title="(block.address != '' || block.city != '' || block.zipcode != '' || block.country != '') ? block.address+' '+block.city+' '+block.zipcode+' '+block.country : ''">
                            <i class="icon dripicons-direction"></i>
                        </a>
                        <a class="col-xs-2 center" :class="(block.email != '') ? 'white' : 'transparent'" :href="(block.email != '') ? 'https://mail.google.com/mail/u/1/?view=cm&fs=1&to='+block.email+'&tf=1' : ''" :title="block.email" target="_blank">
                            <i class="icon dripicons-mail"></i>
                        </a>
                        <a class="col-xs-2 center" :class="(block.phone != '') ? 'white' : 'transparent'" :href="(block.phone != '') ? 'tel:'+normalizePhoneNumber(block.phone) : ''" :title="block.phone">
                            <i class="icon dripicons-phone"></i>
                        </a>
                        <a class="col-xs-2 center" :class="(block.mobile != '') ? 'white' : 'transparent'" :href="(block.mobile != '') ? 'tel:'+normalizePhoneNumber(block.mobile) : ''" :title="block.mobile">
