...
abstract class Connection
{
    ...
    { 
    	...
    	preg_match("/^((?P<scheme>mysql?|mysqli):\/)?\/?((?P<user>.*?)(:(?P<pass>.*?)|)@(?!.*@))?(?P<host>[^:\/\s]+)(?P<port>:([^\/]*))?(?P<path>(\/\w+)*\/)(?P<database>[-\w.]+[^#?\s]*)?(?P<encoding>\?([^#]*))?(?P<fragment>#(.*))?$/", $url, $url_parsed);
        ...
        $parts = array(
        	...
        	"path" => "/".$url_parsed["database"], 
        	"query" => "charset=utf8"
        	);
        if($url_parsed["port"] != ""){
        	$parts["port"] = $url_parsed["port"];
        }
		...
		$url = @mb_parse_url($connection_url);
			if (!isset($url['host']))
